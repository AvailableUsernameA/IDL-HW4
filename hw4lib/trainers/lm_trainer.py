...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        self.criterion = nn.CrossEntropyLoss(weight=None, ignore_index=tokenizer.pad_id, label_smoothing=config['loss']['label_smoothing'])
    ...
    def generate(self, dataloader, generation_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            if generation_config.get('top_k', 0) > 0 or generation_config.get('top_p', 0) > 0:
                ...
                seqs, scores = generator.generate_sample(prompts, temperature=generation_config.get('temperature', 1.0), top_k=generation_config.get('top_k', 0), top_p=generation_config.get('top_p', 0.0))
            elif generation_config.get('beam_width', 1) > 1:
                print("Generating with beam search...")
                seqs, scores = generator.generate_beam(prompts, beam_width=generation_config.get('beam_width', 1), temperature=generation_config.get('temperature', 1.0), repeat_penalty=generation_config.get('repeat_penalty', 1.0))
